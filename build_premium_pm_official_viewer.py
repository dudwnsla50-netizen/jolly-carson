...
CONCEPT_KEYWORDS = {
    ...
    "2-c. 정보시스템 감리 관련 가이드": [
        "발주관리가이드", "감리수행가이드", "유지보수 감리 점검가이드", "점검가이드", "윤리 가이드"
    ],
...
def run_extraction_and_mapping():
    ...
    print("[1/3] 공식범위 기준 PM 기출문제 PDF 파싱 및 분류 DB 구축 중...")
...
def main():
    ...
    local_path = r"e:\jolly-carson\reports\pm_official_scopes.html"
    os.makedirs(os.path.dirname(local_path), exist_ok=True)
    with open(local_path, "w", encoding="utf-8") as f:
        f.write(html_content)
    print(f"[2/3] 로컬 reports 폴더 저장 완료: {local_path}")
    
    artifact_path = r"C:\Users\DCCIS040000\.gemini\antigravity-ide\brain\7e1fd111-1dc1-495d-82a1-c40573600184\pm_official_scopes.html"
    os.makedirs(os.path.dirname(artifact_path), exist_ok=True)
    with open(artifact_path, "w", encoding="utf-8") as f:
        f.write(html_content)
    print(f"[3/3] 아티팩트 디렉토리 저장 완료: {artifact_path}")
    
    print("\n[성공] 초프리미엄 PM 공식범위 기출문제 뷰어 빌드가 완료되었습니다!")
