window.dashboardData = [
  {
    "concept": "네트워크 라우팅 프로토콜 (RIP/OSPF/BGP)",
    "category": "데이터 통신 및 네트워크 설계",
    "count": 30,
    "years": [
      2015,
      2016,
      2017,
      2018,
      2019,
      2020,
      2021,
      2022,
      2023,
      2024,
      2025,
      2026
    ],
    "questions": [
      {
        "year": 2026,
        "num": 94
      },
      {
        "year": 2025,
        "num": 95
      },
      {
        "year": 2025,
        "num": 94
      },
      {
        "year": 2024,
        "num": 99
      },
      {
        "year": 2024,
        "num": 92
      },
      {
        "year": 2024,
        "num": 90
      },
      {
        "year": 2024,
        "num": 86
      },
      {
        "year": 2023,
        "num": 90
      },
      {
        "year": 2023,
        "num": 89
      },
      {
        "year": 2022,
        "num": 91
      },
      {
        "year": 2022,
        "num": 89
      },
      {
        "year": 2022,
        "num": 88
      },
      {
        "year": 2022,
        "num": 82
      },
      {
        "year": 2022,
        "num": 76
      },
      {
        "year": 2021,
        "num": 94
      },
      {
        "year": 2021,
        "num": 93
      },
      {
        "year": 2021,
        "num": 89
      },
      {
        "year": 2021,
        "num": 80
      },
      {
        "year": 2020,
        "num": 100
      },
      {
        "year": 2020,
        "num": 99
      },
      {
        "year": 2020,
        "num": 91
      },
      {
        "year": 2020,
        "num": 80
      },
      {
        "year": 2019,
        "num": 91
      },
      {
        "year": 2018,
        "num": 99
      },
      {
        "year": 2018,
        "num": 98
      },
      {
        "year": 2017,
        "num": 98
      },
      {
        "year": 2016,
        "num": 98
      },
      {
        "year": 2016,
        "num": 89
      },
      {
        "year": 2015,
        "num": 88
      },
      {
        "year": 2015,
        "num": 78
      }
    ],
    "core_concept": "패킷을 목적지까지 가장 효율적인 경로로 전달하기 위한 네트워크 라우팅 프로토콜",
    "features": "RIP(거리 벡터, 벨만-포드, 최대 15홉 제한)와 OSPF(링크 상태, 다익스트라, 계층 구조) 및 BGP(경로 벡터, 자율 시스템 간 연동)의 세부 명세 비교가 출제됩니다.",
    "scope": "데이터 통신 및 네트워크 설계 -> 라우팅 프로토콜",
    "rep_question": "94. 다음 보기에서 설명하고 있는 동적 라우팅 프로토콜로 \n가장 적절한 것은?\n보기\nŸ\n다익스트라(Dijkstra) 알고리즘을 기반으로 하는 \n링크 상태 라우팅 프로토콜이다.\nŸ\nAS(Autonomous System) 내부에서 사용되는 \n내부 게이트웨이 프로토콜에 해당한다. \nŸ\n네트워크의 변화가 있을 때만 라우팅 정보를 \n멀티캐스트로 전송하여 네트워크 대역폭 효율성이 \n높다.\n  ① RIP\n② ARP\n  ③ BGP\n④ OSPF",
    "rep_year": 2026,
    "rep_num": 94
  },
  {
    "concept": "IP 주소 체계 비교 (IPv4 vs IPv6 헤더 필드)",
    "category": "데이터 통신 및 네트워크 설계",
    "count": 27,
    "years": [
      2015,
      2016,
      2017,
      2018,
      2019,
      2020,
      2021,
      2022,
      2023,
      2024,
      2025,
      2026
    ],
    "questions": [
      {
        "year": 2026,
        "num": 97
      },
      {
        "year": 2026,
        "num": 95
      },
      {
        "year": 2026,
        "num": 94
      },
      {
        "year": 2025,
        "num": 82
      },
      {
        "year": 2024,
        "num": 88
      },
      {
        "year": 2023,
        "num": 90
      },
      {
        "year": 2023,
        "num": 87
      },
      {
        "year": 2022,
        "num": 99
      },
      {
        "year": 2022,
        "num": 91
      },
      {
        "year": 2022,
        "num": 90
      },
      {
        "year": 2022,
        "num": 83
      },
      {
        "year": 2022,
        "num": 82
      },
      {
        "year": 2021,
        "num": 100
      },
      {
        "year": 2021,
        "num": 94
      },
      {
        "year": 2020,
        "num": 86
      },
      {
        "year": 2020,
        "num": 80
      },
      {
        "year": 2019,
        "num": 95
      },
      {
        "year": 2019,
        "num": 94
      },
      {
        "year": 2018,
        "num": 99
      },
      {
        "year": 2018,
        "num": 98
      },
      {
        "year": 2018,
        "num": 96
      },
      {
        "year": 2018,
        "num": 83
      },
      {
        "year": 2017,
        "num": 91
      },
      {
        "year": 2017,
        "num": 76
      },
      {
        "year": 2016,
        "num": 97
      },
      {
        "year": 2016,
        "num": 87
      },
      {
        "year": 2015,
        "num": 78
      }
    ],
    "core_concept": "인터넷망 상의 호스트 식별을 위한 주소 규격 및 차세대 인터넷 프로토콜 헤더 사양",
    "features": "IPv4(32비트, 가변 헤더)와 IPv6(128비트, 고정 헤더, 흐름 레이블 필드 도입, 체크섬 필드 제거)의 헤더 필드 대조 및 서브넷 마스크 연산 문제가 빈출됩니다.",
    "scope": "데이터 통신 및 네트워크 설계 -> IP 프로토콜",
    "rep_question": "97. IPv6에서는 확장헤더를 갖고 있다. 다음 중 확장\n헤더로 가장 적절하지 않은 것은?\n  ① Fragmentation\n  ② Destination\n  ③ ESP(Encapsulating Security Payload)\n  ④ Hop Limit",
    "rep_year": 2026,
    "rep_num": 97
  },
  {
    "concept": "TCP 혼잡 제어 (Slow Start / Congestion Avoidance)",
    "category": "데이터 통신 및 네트워크 설계",
    "count": 20,
    "years": [
      2015,
      2016,
      2017,
      2018,
      2020,
      2021,
      2022,
      2023,
      2024,
      2026
    ],
    "questions": [
      {
        "year": 2026,
        "num": 96
      },
      {
        "year": 2026,
        "num": 93
      },
      {
        "year": 2026,
        "num": 92
      },
      {
        "year": 2024,
        "num": 100
      },
      {
        "year": 2024,
        "num": 98
      },
      {
        "year": 2023,
        "num": 100
      },
      {
        "year": 2023,
        "num": 89
      },
      {
        "year": 2023,
        "num": 77
      },
      {
        "year": 2022,
        "num": 83
      },
      {
        "year": 2021,
        "num": 93
      },
      {
        "year": 2020,
        "num": 96
      },
      {
        "year": 2020,
        "num": 79
      },
      {
        "year": 2020,
        "num": 78
      },
      {
        "year": 2018,
        "num": 100
      },
      {
        "year": 2018,
        "num": 96
      },
      {
        "year": 2018,
        "num": 84
      },
      {
        "year": 2017,
        "num": 93
      },
      {
        "year": 2017,
        "num": 84
      },
      {
        "year": 2016,
        "num": 80
      },
      {
        "year": 2015,
        "num": 100
      }
    ],
    "core_concept": "송신 호스트가 네트워크 내의 혼잡 상태를 감지하여 전송률을 스스로 조절하는 흐름 제어 기법",
    "features": "Slow Start(지수적 증가), Congestion Avoidance(선형적 증가), 빠른 재전송(3 Duplicate ACKs 시 임계값 조정 및 전송) 그래프 상의 윈도우 크기 변화를 계산합니다.",
    "scope": "데이터 통신 및 네트워크 설계 -> 전송 프로토콜",
    "rep_question": "96. 다음 보기에서 설명하고 있는 TCP 혼잡 제어 알고리즘의 \n특정 단계로 가장 적절한 것은?\n보기\nŸ\n연결 초기화 또는 타임아웃이 발생한 후 혼잡 \n윈도우 크기를 1로 설정하여 데이터 전송을 \n시작한다. \nŸ\n확인 응답(ACK)을 수신할 때마다 혼잡 윈도우 \n크기를 1씩 증가시킨다. \nŸ\n혼잡 윈도우 크기가 임계값에 도달할 때까지 \n이 방식을 유지한다. \n  ① Fast Retransmission\n② Congestion Avoidance\n  ③ Slow Start\n④ Fast Recovery\n- 20 -",
    "rep_year": 2026,
    "rep_num": 96
  },
  {
    "concept": "CPU 정보 레지스터 및 명령어 사이클",
    "category": "공통기술",
    "count": 19,
    "years": [
      2016,
      2017,
      2018,
      2019,
      2020,
      2022,
      2024,
      2025,
      2026
    ],
    "questions": [
      {
        "year": 2026,
        "num": 86
      },
      {
        "year": 2025,
        "num": 92
      },
      {
        "year": 2025,
        "num": 86
      },
      {
        "year": 2025,
        "num": 84
      },
      {
        "year": 2025,
        "num": 83
      },
      {
        "year": 2024,
        "num": 82
      },
      {
        "year": 2022,
        "num": 97
      },
      {
        "year": 2022,
        "num": 81
      },
      {
        "year": 2020,
        "num": 93
      },
      {
        "year": 2019,
        "num": 97
      },
      {
        "year": 2019,
        "num": 86
      },
      {
        "year": 2019,
        "num": 81
      },
      {
        "year": 2019,
        "num": 79
      },
      {
        "year": 2018,
        "num": 93
      },
      {
        "year": 2018,
        "num": 89
      },
      {
        "year": 2017,
        "num": 100
      },
      {
        "year": 2017,
        "num": 86
      },
      {
        "year": 2017,
        "num": 80
      },
      {
        "year": 2016,
        "num": 80
      }
    ],
    "core_concept": "중앙처리장치의 구성 요소 및 주소 지정 방식과 실행 주기 제어",
    "features": "프로그램 카운터(PC), 누산기(AC), 메모리 주소 레지스터(MAR)의 흐름과 인출(Fetch)->간접(Indirect)->실행(Execute)->인터럽트(Interrupt) 사이클 상태 변화를 묻습니다.",
    "scope": "공통기술 -> 컴퓨터 구조 -> CPU",
    "rep_question": "86. 메모리 계층 구조를 설계하는 근본적인 이유로 \n가장 적절한 것은?\n  ① 속도가 빠르고 비싼 메모리와 속도는 느리지만 \n저렴한 메모리를 조합해 최적의 가성비를 얻기 \n위해 \n  ② 메인 메모리의 전력 소모를 줄이는 것이 유일한 \n목적이기 때문에 \n  ③ 모든 데이터를 가장 빠른 CPU 레지스터에만 \n저장하기 위해 \n  ④ 보조기억장치를 제거하고 캐시만 사용하기 위해",
    "rep_year": 2026,
    "rep_num": 86
  },
  {
    "concept": "OSI 7계층 및 TCP/IP 프로토콜 스택",
    "category": "데이터 통신 및 네트워크 설계",
    "count": 17,
    "years": [
      2015,
      2017,
      2018,
      2019,
      2020,
      2021,
      2022,
      2023,
      2024,
      2026
    ],
    "questions": [
      {
        "year": 2026,
        "num": 95
      },
      {
        "year": 2026,
        "num": 93
      },
      {
        "year": 2026,
        "num": 80
      },
      {
        "year": 2024,
        "num": 79
      },
      {
        "year": 2023,
        "num": 90
      },
      {
        "year": 2023,
        "num": 84
      },
      {
        "year": 2022,
        "num": 80
      },
      {
        "year": 2022,
        "num": 76
      },
      {
        "year": 2021,
        "num": 93
      },
      {
        "year": 2020,
        "num": 78
      },
      {
        "year": 2019,
        "num": 86
      },
      {
        "year": 2018,
        "num": 96
      },
      {
        "year": 2017,
        "num": 95
      },
      {
        "year": 2017,
        "num": 91
      },
      {
        "year": 2017,
        "num": 82
      },
      {
        "year": 2017,
        "num": 76
      },
      {
        "year": 2015,
        "num": 94
      }
    ],
    "core_concept": "네트워크 통신망 구축을 위한 ISO 표준 개방형 아키텍처와 실제 인터넷 프로토콜 표준",
    "features": "각 계층별 역할(데이터링크: 흐름/에러 제어, 전송: End-to-End 신뢰성 등)과 대표적인 프로토콜/장비 매핑 관계를 묻습니다.",
    "scope": "데이터 통신 및 네트워크 설계 -> 네트워크 모델",
    "rep_question": "95. 다음 보기에서 설명하고 있는 응용 계층 프로토콜로 \n가장 적절한 것은?\n보기\nŸ\n네트워크 관리자가 클라이언트에게 수동으로 \nIP 주소를 할당하는 번거로움을 해결하기 위해 \n사용된다. \nŸ\n클라이언트가 네트워크에 접속할 때 IP 주소, \n서브넷 마스크, 기본 게이트웨이 주소 등을 \n동적으로 임대해 준다. \nŸ\nDiscover, Offer, Request, Ack 등의 메시지 \n교환 과정을 거친다.\n  ① FTP\n② SNMP\n  ③ DHCP\n④ NAT",
    "rep_year": 2026,
    "rep_num": 95
  },
  {
    "concept": "클라우드 서비스 모델 (IaaS/PaaS/SaaS) 및 가상화",
    "category": "기타 신기술",
    "count": 17,
    "years": [
      2015,
      2016,
      2018,
      2019,
      2020,
      2021,
      2023,
      2024,
      2026
    ],
    "questions": [
      {
        "year": 2026,
        "num": 100
      },
      {
        "year": 2024,
        "num": 76
      },
      {
        "year": 2023,
        "num": 96
      },
      {
        "year": 2023,
        "num": 95
      },
      {
        "year": 2021,
        "num": 99
      },
      {
        "year": 2021,
        "num": 96
      },
      {
        "year": 2021,
        "num": 91
      },
      {
        "year": 2021,
        "num": 77
      },
      {
        "year": 2020,
        "num": 98
      },
      {
        "year": 2020,
        "num": 82
      },
      {
        "year": 2019,
        "num": 89
      },
      {
        "year": 2019,
        "num": 77
      },
      {
        "year": 2018,
        "num": 85
      },
      {
        "year": 2018,
        "num": 82
      },
      {
        "year": 2016,
        "num": 94
      },
      {
        "year": 2016,
        "num": 80
      },
      {
        "year": 2015,
        "num": 81
      }
    ],
    "core_concept": "가상화된 컴퓨팅 리소스를 온디맨드로 제공하는 서비스 모델 및 컨테이너화 기술",
    "features": "IaaS, PaaS, SaaS의 관리 책임 한계선 분기점 및 Type-1/Type-2 하이퍼바이저와 Docker 컨테이너(OS 커널 공유)의 성능 구조적 장단점을 비교 출제합니다.",
    "scope": "기타 신기술 -> 클라우드 컴퓨팅",
    "rep_question": "100. 도커 컨테이너(Container)와 가상 머신(Virtual Machine)의 \n차이점에 대한 설명으로 가장 적절한 것은?\n  ① 컨테이너는 하드웨어 수준의 가상화를 제공하여 \n보안성이 가상 머신보다 뛰어나다.\n  ② 가상 머신은 하이퍼바이저 없이 작동하며, 컨테\n이너는 반드시 하이퍼바이저가 필요하다.\n  ③ 가상 머신은 호스트 OS의 커널을 공유하지만, \n컨테이너는 독립된 커널을 포함한다.\n  ④ 컨테이너는 가상 머신보다 오버헤드가 적고 시작 \n속도가 매우 빠르다.\n보  안",
    "rep_year": 2026,
    "rep_num": 100
  },
  {
    "concept": "RAID 레벨별 패리티 및 디스크 효율 (0/1/5/6/10)",
    "category": "아키텍처 설계 및 구축",
    "count": 15,
    "years": [
      2017,
      2018,
      2020,
      2021,
      2022,
      2023,
      2024,
      2025,
      2026
    ],
    "questions": [
      {
        "year": 2026,
        "num": 78
      },
      {
        "year": 2025,
        "num": 87
      },
      {
        "year": 2025,
        "num": 81
      },
      {
        "year": 2024,
        "num": 85
      },
      {
        "year": 2024,
        "num": 83
      },
      {
        "year": 2024,
        "num": 81
      },
      {
        "year": 2023,
        "num": 79
      },
      {
        "year": 2022,
        "num": 100
      },
      {
        "year": 2022,
        "num": 87
      },
      {
        "year": 2022,
        "num": 79
      },
      {
        "year": 2021,
        "num": 83
      },
      {
        "year": 2021,
        "num": 76
      },
      {
        "year": 2020,
        "num": 85
      },
      {
        "year": 2018,
        "num": 76
      },
      {
        "year": 2017,
        "num": 92
      }
    ],
    "core_concept": "여러 물리 디스크를 논리적 하나의 저장장치로 묶어 성능과 신뢰성을 올리는 기술",
    "features": "RAID 0(스트라이핑), 1(미러링), 5(분산 단일 패리티), 6(분산 이중 패리티)의 필요 디스크 수 계산, 가용 용량 비율 및 결함 복구 한계를 물어봅니다.",
    "scope": "아키텍처 설계 및 구축 -> 스토리지 아키텍처",
    "rep_question": "78. RAID 구성 방식 중, 데이터를 스트라이핑(Striping)\n하여 저장하면서 별도의 디스크에 패리티(Parity) \n정보를 집중적으로 저장하는 방식으로 가장 적절한 \n것은?\n  ① RAID 0\n② RAID 1\n  ③ RAID 4\n④ RAID 5",
    "rep_year": 2026,
    "rep_num": 78
  },
  {
    "concept": "캐시 메모리 매핑 및 쓰기 정책 (Write-through/Write-back)",
    "category": "공통기술",
    "count": 7,
    "years": [
      2017,
      2019,
      2021,
      2024,
      2025,
      2026
    ],
    "questions": [
      {
        "year": 2026,
        "num": 86
      },
      {
        "year": 2025,
        "num": 85
      },
      {
        "year": 2024,
        "num": 81
      },
      {
        "year": 2021,
        "num": 81
      },
      {
        "year": 2019,
        "num": 87
      },
      {
        "year": 2019,
        "num": 85
      },
      {
        "year": 2017,
        "num": 90
      }
    ],
    "core_concept": "CPU와 메인 메모리 간 속도 차이를 해소하기 위한 고속 버퍼 메모리 운영 기법",
    "features": "직접(Direct), 연관(Associative), 세트 연관(Set-Associative) 매핑의 주소 구조 분석과 Write-through(즉시 기록) / Write-back(교체 시 기록)의 성능 특징을 비교합니다.",
    "scope": "공통기술 -> 컴퓨터 구조 -> 캐시",
    "rep_question": "86. 메모리 계층 구조를 설계하는 근본적인 이유로 \n가장 적절한 것은?\n  ① 속도가 빠르고 비싼 메모리와 속도는 느리지만 \n저렴한 메모리를 조합해 최적의 가성비를 얻기 \n위해 \n  ② 메인 메모리의 전력 소모를 줄이는 것이 유일한 \n목적이기 때문에 \n  ③ 모든 데이터를 가장 빠른 CPU 레지스터에만 \n저장하기 위해 \n  ④ 보조기억장치를 제거하고 캐시만 사용하기 위해",
    "rep_year": 2026,
    "rep_num": 86
  },
  {
    "concept": "가상 메모리 및 페이지 교체 알고리즘 (LRU/LFU/FIFO)",
    "category": "공통기술",
    "count": 7,
    "years": [
      2018,
      2019,
      2021,
      2023,
      2024,
      2025
    ],
    "questions": [
      {
        "year": 2025,
        "num": 86
      },
      {
        "year": 2024,
        "num": 78
      },
      {
        "year": 2023,
        "num": 76
      },
      {
        "year": 2021,
        "num": 81
      },
      {
        "year": 2019,
        "num": 85
      },
      {
        "year": 2018,
        "num": 94
      },
      {
        "year": 2018,
        "num": 93
      }
    ],
    "core_concept": "물리 메모리 크기 한계를 극복하고 가상 주소를 물리 주소로 사상하여 관리하는 메모리 관리 모델",
    "features": "LRU(최근 최소 사용), LFU(최소 빈도 사용), FIFO 교체 알고리즘의 페이지 부재(Page Fault) 횟수 계산 문제 및 스래싱(Thrashing) 예방을 위한 워킹셋 모델을 다룹니다.",
    "scope": "공통기술 -> 운영체제 -> 메모리 관리",
    "rep_question": "86. 다음중가상기억장치에대한설명으로가장\n적절하지않은것은?\n①CPU가\n참조하는\n가상주소를\n주기억\n실제주소로변환하는주소매핑이필요하다.\n②프로그램이\n적재되는\n주기억\n장치의\n물리주소라고한다.\n③가상주소가주기억장치에존재하지않는경우에는\n페이지부재(fault)가발생한다.\n④LFU은최근까지가장오랫동안사용하지않은\n페이지를선택하여교체한다.",
    "rep_year": 2025,
    "rep_num": 86
  },
  {
    "concept": "컴퓨터 조합/순서 논리 회로 (가산기/플립플롭 등)",
    "category": "공통기술",
    "count": 5,
    "years": [
      2023,
      2024
    ],
    "questions": [
      {
        "year": 2024,
        "num": 95
      },
      {
        "year": 2024,
        "num": 94
      },
      {
        "year": 2024,
        "num": 79
      },
      {
        "year": 2024,
        "num": 77
      },
      {
        "year": 2023,
        "num": 98
      }
    ],
    "core_concept": "하드웨어 회로 설계를 위한 기초 수학 논리 및 소자 아키텍처",
    "features": "가산기, 디코더, 멀티플렉서(기억 소자 없음 -> 조합 회로)와 플립플롭, 카운터(기억 소자 있음 -> 순서 회로)의 출력 진리표 해석과 부울 대수 간소화를 질문합니다.",
    "scope": "공통기술 -> 디지털 논리 설계",
    "rep_question": "95. BERT(Bidirectional Encoder Representation from \nTransformer)에 대한 설명으로 가장 적절하지 않은 \n것은?\n  ① 위치 임베딩을 사용한다.\n  ② 인코더와 디코더를 갖는다.\n  ③ 트랜스포머 모델을 기반으로 한다.\n  ④ 자연어 처리를 위한 딥러닝 아키텍처 중 하나이다.",
    "rep_year": 2024,
    "rep_num": 95
  },
  {
    "concept": "고가용성 아키텍처 및 재해 복구 시스템 (DRS)",
    "category": "아키텍처 설계 및 구축",
    "count": 4,
    "years": [
      2016,
      2019,
      2020,
      2026
    ],
    "questions": [
      {
        "year": 2026,
        "num": 76
      },
      {
        "year": 2020,
        "num": 99
      },
      {
        "year": 2019,
        "num": 78
      },
      {
        "year": 2016,
        "num": 78
      }
    ],
    "core_concept": "시스템 다운 타임을 최소화하기 위한 이중화 및 재해 복구(DR) 아키텍처 표준",
    "features": "Active-Active, Active-Standby 구성의 세션 동기화 문제와 DRS 수준(Mirroring, Hot, Warm, Cold)에 따른 RTO(복구목표시간) 및 RPO(복구목표시점) 관계를 매년 질문합니다.",
    "scope": "아키텍처 설계 및 구축 -> 가용성 설계",
    "rep_question": "76. 재해복구 설계에서 데이터 손실 허용 범위를 시간 \n단위로 정의하는 목표 지표로 가장 적절한 것은?\n  ① RPO(Recovery Point Objective)\n  ② RTO(Recovery Time Objective)\n  ③ MTBF(Mean Time Between Failures)\n  ④ MTTR(Mean Time To Repair)",
    "rep_year": 2026,
    "rep_num": 76
  },
  {
    "concept": "명령어 파이프라이닝 및 해저드 종류 (구조적/데이터/제어)",
    "category": "공통기술",
    "count": 3,
    "years": [
      2017,
      2021,
      2022
    ],
    "questions": [
      {
        "year": 2022,
        "num": 84
      },
      {
        "year": 2021,
        "num": 91
      },
      {
        "year": 2017,
        "num": 86
      }
    ],
    "core_concept": "여러 명령어를 중첩 실행하여 처리량(Throughput)을 높이는 병렬 처리 기술",
    "features": "구조적 해저드(자원 충돌), 데이터 해저드(RAW, WAR, WAW 의존성), 제어 해저드(분기 명령 발생)의 발생 원인과 지연(Stall) 및 우회(Bypassing) 해결 기법을 출제합니다.",
    "scope": "공통기술 -> 컴퓨터 구조 -> 파이프라인",
    "rep_question": "84. 명령어 파이프라인은 해저드(Hazard)라는 중대한 \n문제를 발생시킨다. 다음 중 일반적인 파이프라인 \n해저드의 종류로 가장 적절하지 않은 것은?\n  ① Structural Hazard\n② Register Hazard\n  ③ Data Hazard\n④ Control Hazard",
    "rep_year": 2022,
    "rep_num": 84
  },
  {
    "concept": "[기타]",
    "category": "기타",
    "count": 164,
    "years": [
      2015,
      2016,
      2017,
      2018,
      2019,
      2020,
      2021,
      2022,
      2023,
      2024,
      2025,
      2026
    ],
    "questions": [
      {
        "year": 2026,
        "num": 99
      },
      {
        "year": 2026,
        "num": 98
      },
      {
        "year": 2026,
        "num": 91
      },
      {
        "year": 2026,
        "num": 90
      },
      {
        "year": 2026,
        "num": 89
      },
      {
        "year": 2026,
        "num": 88
      },
      {
        "year": 2026,
        "num": 87
      },
      {
        "year": 2026,
        "num": 85
      },
      {
        "year": 2026,
        "num": 84
      },
      {
        "year": 2026,
        "num": 83
      },
      {
        "year": 2026,
        "num": 82
      },
      {
        "year": 2026,
        "num": 81
      },
      {
        "year": 2026,
        "num": 79
      },
      {
        "year": 2026,
        "num": 77
      },
      {
        "year": 2025,
        "num": 100
      },
      {
        "year": 2025,
        "num": 99
      },
      {
        "year": 2025,
        "num": 98
      },
      {
        "year": 2025,
        "num": 97
      },
      {
        "year": 2025,
        "num": 96
      },
      {
        "year": 2025,
        "num": 93
      },
      {
        "year": 2025,
        "num": 91
      },
      {
        "year": 2025,
        "num": 90
      },
      {
        "year": 2025,
        "num": 89
      },
      {
        "year": 2025,
        "num": 88
      },
      {
        "year": 2025,
        "num": 80
      },
      {
        "year": 2025,
        "num": 79
      },
      {
        "year": 2025,
        "num": 78
      },
      {
        "year": 2025,
        "num": 77
      },
      {
        "year": 2025,
        "num": 76
      },
      {
        "year": 2024,
        "num": 97
      },
      {
        "year": 2024,
        "num": 96
      },
      {
        "year": 2024,
        "num": 93
      },
      {
        "year": 2024,
        "num": 91
      },
      {
        "year": 2024,
        "num": 89
      },
      {
        "year": 2024,
        "num": 87
      },
      {
        "year": 2024,
        "num": 84
      },
      {
        "year": 2024,
        "num": 80
      },
      {
        "year": 2023,
        "num": 99
      },
      {
        "year": 2023,
        "num": 97
      },
      {
        "year": 2023,
        "num": 94
      },
      {
        "year": 2023,
        "num": 93
      },
      {
        "year": 2023,
        "num": 92
      },
      {
        "year": 2023,
        "num": 91
      },
      {
        "year": 2023,
        "num": 88
      },
      {
        "year": 2023,
        "num": 86
      },
      {
        "year": 2023,
        "num": 85
      },
      {
        "year": 2023,
        "num": 83
      },
      {
        "year": 2023,
        "num": 82
      },
      {
        "year": 2023,
        "num": 81
      },
      {
        "year": 2023,
        "num": 80
      },
      {
        "year": 2023,
        "num": 78
      },
      {
        "year": 2022,
        "num": 98
      },
      {
        "year": 2022,
        "num": 96
      },
      {
        "year": 2022,
        "num": 95
      },
      {
        "year": 2022,
        "num": 94
      },
      {
        "year": 2022,
        "num": 93
      },
      {
        "year": 2022,
        "num": 92
      },
      {
        "year": 2022,
        "num": 86
      },
      {
        "year": 2022,
        "num": 85
      },
      {
        "year": 2022,
        "num": 78
      },
      {
        "year": 2022,
        "num": 77
      },
      {
        "year": 2021,
        "num": 98
      },
      {
        "year": 2021,
        "num": 97
      },
      {
        "year": 2021,
        "num": 95
      },
      {
        "year": 2021,
        "num": 92
      },
      {
        "year": 2021,
        "num": 90
      },
      {
        "year": 2021,
        "num": 88
      },
      {
        "year": 2021,
        "num": 87
      },
      {
        "year": 2021,
        "num": 86
      },
      {
        "year": 2021,
        "num": 85
      },
      {
        "year": 2021,
        "num": 84
      },
      {
        "year": 2021,
        "num": 82
      },
      {
        "year": 2021,
        "num": 79
      },
      {
        "year": 2021,
        "num": 78
      },
      {
        "year": 2020,
        "num": 97
      },
      {
        "year": 2020,
        "num": 95
      },
      {
        "year": 2020,
        "num": 94
      },
      {
        "year": 2020,
        "num": 92
      },
      {
        "year": 2020,
        "num": 90
      },
      {
        "year": 2020,
        "num": 89
      },
      {
        "year": 2020,
        "num": 88
      },
      {
        "year": 2020,
        "num": 87
      },
      {
        "year": 2020,
        "num": 84
      },
      {
        "year": 2020,
        "num": 83
      },
      {
        "year": 2020,
        "num": 81
      },
      {
        "year": 2020,
        "num": 77
      },
      {
        "year": 2020,
        "num": 76
      },
      {
        "year": 2019,
        "num": 100
      },
      {
        "year": 2019,
        "num": 99
      },
      {
        "year": 2019,
        "num": 98
      },
      {
        "year": 2019,
        "num": 96
      },
      {
        "year": 2019,
        "num": 93
      },
      {
        "year": 2019,
        "num": 92
      },
      {
        "year": 2019,
        "num": 90
      },
      {
        "year": 2019,
        "num": 88
      },
      {
        "year": 2019,
        "num": 84
      },
      {
        "year": 2019,
        "num": 83
      },
      {
        "year": 2019,
        "num": 82
      },
      {
        "year": 2019,
        "num": 80
      },
      {
        "year": 2019,
        "num": 76
      },
      {
        "year": 2018,
        "num": 97
      },
      {
        "year": 2018,
        "num": 95
      },
      {
        "year": 2018,
        "num": 92
      },
      {
        "year": 2018,
        "num": 91
      },
      {
        "year": 2018,
        "num": 90
      },
      {
        "year": 2018,
        "num": 88
      },
      {
        "year": 2018,
        "num": 87
      },
      {
        "year": 2018,
        "num": 86
      },
      {
        "year": 2018,
        "num": 81
      },
      {
        "year": 2018,
        "num": 80
      },
      {
        "year": 2018,
        "num": 79
      },
      {
        "year": 2018,
        "num": 78
      },
      {
        "year": 2018,
        "num": 77
      },
      {
        "year": 2017,
        "num": 99
      },
      {
        "year": 2017,
        "num": 97
      },
      {
        "year": 2017,
        "num": 96
      },
      {
        "year": 2017,
        "num": 94
      },
      {
        "year": 2017,
        "num": 89
      },
      {
        "year": 2017,
        "num": 88
      },
      {
        "year": 2017,
        "num": 87
      },
      {
        "year": 2017,
        "num": 85
      },
      {
        "year": 2017,
        "num": 83
      },
      {
        "year": 2017,
        "num": 81
      },
      {
        "year": 2017,
        "num": 79
      },
      {
        "year": 2017,
        "num": 78
      },
      {
        "year": 2017,
        "num": 77
      },
      {
        "year": 2016,
        "num": 100
      },
      {
        "year": 2016,
        "num": 99
      },
      {
        "year": 2016,
        "num": 96
      },
      {
        "year": 2016,
        "num": 95
      },
      {
        "year": 2016,
        "num": 93
      },
      {
        "year": 2016,
        "num": 92
      },
      {
        "year": 2016,
        "num": 91
      },
      {
        "year": 2016,
        "num": 90
      },
      {
        "year": 2016,
        "num": 88
      },
      {
        "year": 2016,
        "num": 86
      },
      {
        "year": 2016,
        "num": 85
      },
      {
        "year": 2016,
        "num": 84
      },
      {
        "year": 2016,
        "num": 83
      },
      {
        "year": 2016,
        "num": 82
      },
      {
        "year": 2016,
        "num": 81
      },
      {
        "year": 2016,
        "num": 79
      },
      {
        "year": 2016,
        "num": 77
      },
      {
        "year": 2016,
        "num": 76
      },
      {
        "year": 2015,
        "num": 99
      },
      {
        "year": 2015,
        "num": 98
      },
      {
        "year": 2015,
        "num": 97
      },
      {
        "year": 2015,
        "num": 96
      },
      {
        "year": 2015,
        "num": 95
      },
      {
        "year": 2015,
        "num": 93
      },
      {
        "year": 2015,
        "num": 92
      },
      {
        "year": 2015,
        "num": 91
      },
      {
        "year": 2015,
        "num": 90
      },
      {
        "year": 2015,
        "num": 89
      },
      {
        "year": 2015,
        "num": 87
      },
      {
        "year": 2015,
        "num": 86
      },
      {
        "year": 2015,
        "num": 85
      },
      {
        "year": 2015,
        "num": 84
      },
      {
        "year": 2015,
        "num": 83
      },
      {
        "year": 2015,
        "num": 82
      },
      {
        "year": 2015,
        "num": 80
      },
      {
        "year": 2015,
        "num": 79
      },
      {
        "year": 2015,
        "num": 77
      },
      {
        "year": 2015,
        "num": 76
      }
    ],
    "core_concept": "기출문제 중 기존 세부 개념으로 정의되지 않은 기타 문항들입니다.",
    "features": "기타 분류된 문제 리스트를 검토하여 누락된 개념이나 특이 문제 유형을 점검합니다.",
    "scope": "기타",
    "rep_question": "99. 사전 학습된 대규모 모델은 이미 방대한 데이터로 \n학습했지만, 새로운 데이터를 처리하거나 특정 \n작업에 맞추려면 추가적인 파인튜팅이 필요하다. \n사전 학습된 모델의 구조를 그대로 유지하면서 \n필요한 만큼만 파라미터를 효율적으로 수정하는 \n기술로 가장 적절한 것은?\n  ① RLHF(Reinforcement Learning from Human Feedback)\n  ② LoRA(Low-Rank Adaptation)\n  ③ RAG(Retrieval-Augmented Generation)\n  ④ Few-shot Prompting",
    "rep_year": 2026,
    "rep_num": 99
  }
];
